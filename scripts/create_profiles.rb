...
begin
  ...
  # Get the bundle ID
  FastlaneLogger.info("📱 Finding Bundle ID for com.yourcompany.yourapp...")
  bundle_ids = Spaceship::ConnectAPI::BundleId.all
  bundle_id = bundle_ids.find { |bid| bid.identifier == "com.yourcompany.yourapp" }
  
  if bundle_id.nil?
    FastlaneLogger.warn("Bundle ID com.yourcompany.yourapp not found. Creating it...")
  ...
  if dev_cert
    ...
    begin
      dev_profile = Spaceship::ConnectAPI::Profile.create(
        name: "com.yourcompany.yourapp Development",
  ...
  if dist_cert
    ...
    begin
      dist_profile = Spaceship::ConnectAPI::Profile.create(
        name: "com.yourcompany.yourapp AppStore",
