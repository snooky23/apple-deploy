...
begin
  ...
  voice_forms_profiles = profiles.select { |p| p.name.include?("Voice Forms") || p.name.include?("com.yourcompany.yourapp") }
  ...
  voice_forms_profiles.each do |profile|
    ...
    begin
      ...
      system("fastlane run download_provisioning_profiles app_identifier:com.yourcompany.yourapp team_id:YOUR_TEAM_ID output_path:#{profiles_dir}")
